using System;

namespace SecurityLibrary.AES
{
    /// <summary>
    /// If the string starts with 0x.... then it's Hexadecimal not string
    /// </summary>
    public class AES : CryptographicTechnique
    {
        #region Fields
        private readonly byte[] sBox = new byte[]
        {
                
        };
        private readonly byte[] inverseSBox = new byte[]
        {
            
        };
        private readonly byte[] mixColumnsMatrix = new byte[16]
        {
            0x02, 0x03, 0x01, 0x01,
            0x01, 0x02, 0x03, 0x01,
            0x01, 0x01, 0x02, 0x03,
            0x03, 0x01, 0x01, 0x02
        };
        private readonly byte[] inverseMixColumnsMatrix = new byte[16]
        {
            0x0E, 0x0B, 0x0D, 0x09,
            0x09, 0x0E, 0x0B, 0x0D,
            0x0D, 0x09, 0x0E, 0x0B,
            0x0B, 0x0D, 0x09, 0x0E
        };
        private string RconMatrix = "01000000020000000400000008000000100000002000000040000000800000001B00000036000000";
        #endregion

        #region Decryption
        public override string Decrypt(string cipherText, string key)
        {
            // Removing the "0x" prefix
            cipherText = cipherText.Substring(2);
            key = key.Substring(2);

            // Previous key used in Key Schedule step
            string previousKey = key;

            // Getting all the keys
            string[] currentKey = new string[10];
            for (int i = 0; i < 10; i++)
            {
                currentKey[i] = KeySchedule(previousKey, i);
                previousKey = currentKey[i];
            }

            // Initial Round
            string state = AddRoundKey(cipherText, currentKey[9]);
            string shiftRowsResult = InverseShiftRows(state);
            string subBytesResult = InverseSubBytes(shiftRowsResult);
            state = subBytesResult;

            // Main Rounds
            for (int i = 8; i >= 0; i--)
            {
                // AddRoundKey
                string addRoundKeyResult = AddRoundKey(state, currentKey[i]);
                // Mix Columns
                string mixColumnsResult = InverseMixColumns(addRoundKeyResult);
                // Shift Rows
                shiftRowsResult = InverseShiftRows(mixColumnsResult);
                // Substitute bytes
                subBytesResult = InverseSubBytes(shiftRowsResult);
                state = subBytesResult;
            }

            string plainText = AddRoundKey(state, key);

            return "0x" + plainText;
        }
        public string InverseSubBytes(string state)
        {
            byte[] cells = new byte[state.Length / 2];
            for (int i = 0; i < state.Length / 2; i++)
            {
                cells[i] = Convert.ToByte(state.Substring(2 * i, 2), 16);
            }

            // Perform SubBytes operation
            for (int i = 0; i < state.Length / 2; i++)
            {
                cells[i] = inverseSBox[cells[i]];
            }

            // Convert byte array back to string
            string output = BitConverter.ToString(cells).Replace("-", "");

            return output;
        }
        public string InverseShiftRows(string state)
        {
            state = Rotate(state);
            string shiftedState = "";

            for (int row = 0; row < 4; row++)
            {
                string rowState = state.Substring(row * 8, 8);
                string shiftedRow = rowState.Substring((4 - row) * 2) + rowState.Substring(0, (4 - row) * 2);
                shiftedState += shiftedRow;
            }
            shiftedState = Rotate(shiftedState);
            return shiftedState;
        }
        public string InverseMixColumns(string state)
        {
            // Getting the MixColumns matrix 
            string result = "";
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    byte a0 = Convert.ToByte(state.Substring(i * 8, 2), 16);
                    byte a1 = Convert.ToByte(state.Substring(i * 8 + 2, 2), 16);
                    byte a2 = Convert.ToByte(state.Substring(i * 8 + 4, 2), 16);
                    byte a3 = Convert.ToByte(state.Substring(i * 8 + 6, 2), 16);

                    byte multipliedCell = (byte)(GFMultiply(inverseMixColumnsMatrix[j * 4], a0) ^
                                                 GFMultiply(inverseMixColumnsMatrix[j * 4 + 1], a1) ^
                                                 GFMultiply(inverseMixColumnsMatrix[j * 4 + 2], a2) ^
                                                 GFMultiply(inverseMixColumnsMatrix[j * 4 + 3], a3));
                    result += multipliedCell.ToString("X2");
                }
            }

            return result;
        }

        #endregion

        #region Encryption
        public override string Encrypt(string plainText, string key)
        {
            plainText = plainText.Substring(2);
            key = key.Substring(2);

            // Previous key used in Key Schedule step
            string previousKey = key;

            // Initial Round
            string state = AddRoundKey(plainText, key);

            // Main Rounds
            for (int i = 0; i < 9; i++)
            {
                // Substitute bytes
                string subBytesResult = SubBytes(state);
                // Shift Rows
                string shiftRowsResult = ShiftRows(subBytesResult);
                // Mix Columns
                string mixColumnsResult = MixColumns(shiftRowsResult);
                // Key Scedule
                string keySceduleResult = KeySchedule(previousKey, i);
                previousKey = keySceduleResult;
                // AddRoundKey
                string addRoundKeyResult = AddRoundKey(mixColumnsResult, keySceduleResult);
                state = addRoundKeyResult;
            }

            // Final Round
            string finalRoundSubBytesResult = SubBytes(state);
            string finalRoundshiftRowsResult = ShiftRows(finalRoundSubBytesResult);
            string finalRoundkeySceduleResult = KeySchedule(previousKey, 9);
            string cipherText = AddRoundKey(finalRoundshiftRowsResult, finalRoundkeySceduleResult);

            return "0x" + cipherText;
        }
        public string AddRoundKey(string state, string key)
        {
            string result = "";
            for (int i = 0; i < state.Length; i += 2)
            {
                int stateCell = Convert.ToInt32(state.Substring(i, 2), 16);
                int keyCell = Convert.ToInt32(key.Substring(i, 2), 16);

                int xorResult = stateCell ^ keyCell;

                result += xorResult.ToString("X2");
            }
            return result;
        }
        private string SubBytes(string state)
        {
            byte[] cells = new byte[state.Length / 2];
            for (int i = 0; i < state.Length / 2; i++)
            {
                cells[i] = Convert.ToByte(state.Substring(2 * i, 2), 16);
            }

            // Perform SubBytes operation
            for (int i = 0; i < state.Length / 2; i++)
            {
                cells[i] = sBox[cells[i]];
            }

            // Convert byte array back to string
            string output = BitConverter.ToString(cells).Replace("-", "");

            return output;
        }
        private string ShiftRows(string state)
        {
            state = Rotate(state);
            string shiftedState = "";

            for (int row = 0; row < 4; row++)
            {
                string rowState = state.Substring(row * 8, 8);
                string shiftedRow = rowState.Substring(row * 2) + rowState.Substring(0, row * 2);
                shiftedState += shiftedRow;
            }
            shiftedState = Rotate(shiftedState);
            return shiftedState;
        }
        private string MixColumns(string state)
        {
            // Getting the MixColumns matrix 
            string result = "";
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    byte a0 = Convert.ToByte(state.Substring(i * 8, 2), 16);
                    byte a1 = Convert.ToByte(state.Substring(i * 8 + 2, 2), 16);
                    byte a2 = Convert.ToByte(state.Substring(i * 8 + 4, 2), 16);
                    byte a3 = Convert.ToByte(state.Substring(i * 8 + 6, 2), 16);

                    byte multipliedCell = (byte)(GFMultiply(mixColumnsMatrix[j * 4], a0) ^
                                                 GFMultiply(mixColumnsMatrix[j * 4 + 1], a1) ^
                                                 GFMultiply(mixColumnsMatrix[j * 4 + 2], a2) ^
                                                 GFMultiply(mixColumnsMatrix[j * 4 + 3], a3));
                    result += multipliedCell.ToString("X2");
                }
            }

            return result;
        }
        private string KeySchedule(string key, int roundNumber)
        {
            // Getting each column alone
            string c1 = key.Substring(0, 8),
                   c2 = key.Substring(8, 8),
                   c3 = key.Substring(16, 8),
                   c4 = key.Substring(24, 8);

            // Rotate the last column
            string tmp = RotateWord(c4);
            // Applying Subbytes to the last column
            tmp = SubBytes(tmp);
            // Doing XOR between first, last and racon columns
            tmp = AddRoundKey(c1, tmp);
            tmp = AddRoundKey(tmp, RconMatrix.Substring(roundNumber * 8, 8));
            // Getting the new key
            c1 = tmp;
            c2 = AddRoundKey(c1, c2);
            c3 = AddRoundKey(c2, c3);
            c4 = AddRoundKey(c3, c4);

            return c1 + c2 + c3 + c4;
        }
        #endregion

        #region Helper Functions
        private string Rotate(string matrix)
        {
            string r1 = "", r2 = "", r3 = "", r4 = "";
            for (int i = 0; i < 32; i += 8)
            {
                r1 += (matrix[i]);
                r1 += (matrix[i + 1]);
                r2 += (matrix[i + 2]);
                r2 += (matrix[i + 3]);
                r3 += (matrix[i + 4]);
                r3 += (matrix[i + 5]);
                r4 += (matrix[i + 6]);
                r4 += (matrix[i + 7]);
            }
            matrix = r1 + r2 + r3 + r4;
            return matrix;
        }
        private byte GFMultiply(byte a, byte b)
        {

            byte p = 0;
            byte mask = 0x01; // Mask for checking each bit of b
            for (int i = 0; i < 8; i++)
            {
                if ((b & mask) != 0) // Check if the current bit of b is set
                {
                    p ^= a; // If set, XOR a with p
                }
                byte highBit = (byte)(a & 0x80); // Check if the high bit of a is 0
                a <<= 1; // Left shift a
                if (highBit != 0) // If the high bit was 1, perform reduction
                {
                    a ^= 0x1B; // XOR with irreducible polynomial
                }
                mask <<= 1; // Shift the mask to the left for the next bit of b
            }
            return p;
        }
        private string RotateWord(string key)
        {
            return key.Substring(2) + key[0] + key[1];
        }
        #endregion

    }
}
